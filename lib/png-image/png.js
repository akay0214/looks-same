    ...
    highlightPixel(x, y, width, height, color, transparency, spread) {
        ...
        for (let i = 0; i <= spread; i++) {
            for (let j = 0; j <= spread - i; j++) {
                ...
                //if (xTopRight < width)
                this.setPixel(xTopRight, yTopRight, color, transparency);
                xBottomRight = x + i; yBottomRight = y - j;
                //if (xBottomRight < width && yBottomRight < height)
                this.setPixel(xBottomRight, yBottomRight, color, transparency);
                xBottomLeft = x - i; yBottomLeft = y - j;
                //if (xBottomLeft > 0 && yBottomLeft < height)
                this.setPixel(xBottomLeft, yBottomLeft, color, transparency);
                xTopLeft = x - i; yTopLeft = y + j;
                //if (xTopLeft > 0 && yTopLeft < height)
                this.setPixel(xTopLeft, yTopLeft, color, transparency);
            }
        }
    }
