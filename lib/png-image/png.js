...
module.exports = class PNGImage extends PNGBase {
    ...
    setPixel(x, y, color, transparencyPercentage) {
        const idx = this._getIdx(x, y);
        this._png.data[idx] = color.R;
        this._png.data[idx + 1] = color.G;
        this._png.data[idx + 2] = color.B;
        if (transparencyPercentage !== undefined && transparencyPercentage !== null && !isNaN(transparencyPercentage)) {
            this._png.data[idx + 3] = Math.round((255 / transparencyPercentage) * 100);
        } else {
